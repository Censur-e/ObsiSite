...
export async function listBlacklist(profileId) {
  return await sb(`blacklist?profile_id=eq.${encodeURIComponent(profileId)}&order=created_at.desc`)
}

export async function getBlacklistEntry(profileId, playerId) {
  const rows = await sb(`blacklist?profile_id=eq.${encodeURIComponent(profileId)}&player_id=eq.${encodeURIComponent(playerId)}&limit=1`)
  return rows && rows[0] ? rows[0] : null
}

export async function insertBlacklist(row) {
  const rows = await sb('blacklist', { method: 'POST', body: row })
  return rows && rows[0] ? rows[0] : null
}

export async function deleteBlacklist(profileId, id) {
  return await sb(`blacklist?id=eq.${encodeURIComponent(id)}&profile_id=eq.${encodeURIComponent(profileId)}`, { method: 'DELETE' })
}
