...
export async function listBlacklist() {
  return await sb(`blacklist?order=created_at.desc`)
}

export async function getBlacklistEntry(playerId) {
  const rows = await sb(`blacklist?player_id=eq.${encodeURIComponent(playerId)}&limit=1`)
  return rows && rows[0] ? rows[0] : null
}
...
export async function deleteBlacklist(id) {
  return await sb(`blacklist?id=eq.${encodeURIComponent(id)}`, { method: 'DELETE' })
}
